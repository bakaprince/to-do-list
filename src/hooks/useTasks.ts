  ...
  const updateTask = useCallback(
    async (id: string, updates: { title?: string; priority?: Priority; description?: string }) => {
      ...
      if (updates.title !== undefined) {
        ...
        if (cleanTitle.length > 200) throw new Error('Task title cannot exceed 200 characters.');
      ...
      if (updates.description !== undefined) {
        ...
        if (cleanDesc.length > 1000) throw new Error('Task description cannot exceed 1000 characters.');
        payload.description = cleanDesc;
      }
