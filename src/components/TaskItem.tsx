...
interface TaskItemProps {
    ...
    onUpdate: (id: string, updates: { title?: string; priority?: Priority; description?: string; visibility?: TaskVisibility }) => Promise<void>;
}
...
export const TaskItem: React.FC<TaskItemProps> = ({
    ...
    const [editVisibility, setEditVisibility] = useState<TaskVisibility>(task.visibility || 'private');
    ...
    const handleCancelEdit = () => {
        ...
        setEditVisibility(task.visibility || 'private');
        setIsEditing(false);
    };
    ...
    return (
        <div
            ...
            className={`group rounded-[28px] p-5 sm:p-6 transition-all duration-200 ${task.completed
            ...
            {isEditing ? (
                <div className="space-y-3.5">
                    ...
                    <div className="flex items-center justify-between gap-2 pt-1">
                        <div className="flex items-center gap-1.5">
                            {(['low', 'medium', 'high'] as Priority[]).map((p) => (
                                <button
                                    ...
                                    className={`text-xs px-3 py-1 rounded-xl capitalize font-medium border transition-all ${editPriority === p
                        ...
                        <label className="text-xs text-[#8C7E6F]">
                            <span className="mr-2">Visibility</span>
                            <select
                                value={editVisibility}
                                onChange={(e) => setEditVisibility(e.target.value as TaskVisibility)}
                                className="rounded-xl border border-[#D9CFC4] bg-[#FDFCF9] px-2 py-1.5 text-xs text-[#3E362E]"
                            >
                ...
                <div className="flex items-center gap-4 sm:gap-5">
                    ...
                    <button
                        ...
                        className={`w-7 h-7 rounded-full flex items-center justify-center transition-all duration-200 border-2 shrink-0 cursor-pointer ${task.completed
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2.5">
                            <span
                                onClick={() => onToggle(task.id, task.completed)}
                                className={`text-base font-semibold cursor-pointer break-words transition-colors ${task.completed
                            ...
                            {task.visibility === 'friends' && (
                                <span className="inline-flex items-center gap-1 rounded-full border border-[#5D6D5E]/25 bg-[#F0F4F2] px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-[#5D6D5E]">
